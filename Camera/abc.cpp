//Execute with:   g++ abc.cpp -o 2cams `pkg-config --cflags --libs opencv`
...
cv::VideoCapture cap2("v4l2src device=\"/dev/video1\" ! video/x-raw, width=640, height=480, format=RGB ! videoconvert ! appsink");
    ...
    writer1.open("appsrc ! videoconvert ! x264enc noise-reduction=10000 tune=zerolatency byte-stream=true bitrate=3000 threads=2 ! h264parse config-interval=1 ! rtph264pay ! udpsink host=192.168.1.5 port=5002", 0, (double)30, cv::Size(1280, 480), true);
    if (!writer1.isOpened()) {
...
both = Mat(height , (width * 2), CV_MAKETYPE(8, 3), CV_RGB(100, 100, 100));
